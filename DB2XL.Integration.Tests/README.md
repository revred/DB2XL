# DB2XL.Integration.Tests - DB2XL Test Harness
...
```bash
dotnet test DB2XL.Integration.Tests -- -k
```
Same as above but retains the generated .db and .xlsx files.

### Export Existing Database
```bash
dotnet test DB2XL.Integration.Tests -- -d mydata.db
```
Exports an existing SQLite database and validates the result.

### Full Control
```bash
dotnet test DB2XL.Integration.Tests -- -d input.db -e output.xlsx -k
```
