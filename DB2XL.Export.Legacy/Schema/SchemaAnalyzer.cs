...
public static class SchemaAnalyzer
{
        ...
        TransformationPipeline? transformationPipeline = null)
    {
        ...
        countCmd.CommandText = $"SELECT COUNT(*) FROM {SqlQueryBuilder.QuoteIdentifier(table.Name)}";
        ...
        TransformationPipeline? transformationPipeline = null)
    {
        ...
        try
        {
            ...
            // Count nulls
            cmd.CommandText = $"SELECT COUNT(*) FROM {SqlQueryBuilder.QuoteIdentifier(tableName)} WHERE {SqlQueryBuilder.QuoteIdentifier(column.Name)} IS NULL";
            columnSchema.NullCount = Convert.ToInt64(cmd.ExecuteScalar() ?? 0);
            
            // Count non-nulls
            cmd.CommandText = $"SELECT COUNT(*) FROM {SqlQueryBuilder.QuoteIdentifier(tableName)} WHERE {SqlQueryBuilder.QuoteIdentifier(column.Name)} IS NOT NULL";
            columnSchema.NonNullCount = Convert.ToInt64(cmd.ExecuteScalar() ?? 0);
            
            // Get distinct count (limited to avoid performance issues)
            cmd.CommandText = $"SELECT COUNT(DISTINCT {SqlQueryBuilder.QuoteIdentifier(column.Name)}) FROM {SqlQueryBuilder.QuoteIdentifier(tableName)} LIMIT 10000";
            ...
            if (column.Type.ToUpperInvariant().Contains("TEXT") || column.Type.ToUpperInvariant().Contains("VARCHAR"))
            {
                cmd.CommandText = $@"
                    SELECT 
                        MIN(LENGTH({SqlQueryBuilder.QuoteIdentifier(column.Name)})) as MinLength,
                        MAX(LENGTH({SqlQueryBuilder.QuoteIdentifier(column.Name)})) as MaxLength,
                        AVG(LENGTH({SqlQueryBuilder.QuoteIdentifier(column.Name)})) as AvgLength
                    FROM {SqlQueryBuilder.QuoteIdentifier(tableName)} 
                    WHERE {SqlQueryBuilder.QuoteIdentifier(column.Name)} IS NOT NULL";
            ...
            if (IsNumericType(column.Type))
            {
                cmd.CommandText = $@"
                    SELECT 
                        MIN({SqlQueryBuilder.QuoteIdentifier(column.Name)}) as MinValue,
                        MAX({SqlQueryBuilder.QuoteIdentifier(column.Name)}) as MaxValue,
                        AVG({SqlQueryBuilder.QuoteIdentifier(column.Name)}) as AvgValue
                    FROM {SqlQueryBuilder.QuoteIdentifier(tableName)} 
                    WHERE {SqlQueryBuilder.QuoteIdentifier(column.Name)} IS NOT NULL";
